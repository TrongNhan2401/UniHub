    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#0f172a" }}>
      ...
      <View style={s.cameraArea}>
        ...
        <View style={s.topBar}>
          ...
          <TouchableOpacity onPress={() => setFlash(!flash)} style={[s.iconBtn, flash && { backgroundColor: "#fbbf24" }]}>
          ...
          {recentScans.map((item, i) => (
            <View key={item.id} style={[s.scanRow, i > 0 && { borderTopWidth: 1, borderColor: "#f1f5f9" }]}>
              <View style={[s.avatarCircle, { backgroundColor: item.status === "offline" ? "#fee2e2" : "#dbeafe" }]}>
                <Text style={{ fontWeight: "700", fontSize: 13, color: item.status === "offline" ? "#dc2626" : "#2563eb" }}>
                  {item.name.split(" ").map(n => n[0]).join("")}
                </Text>
              </View>
              <View style={{ flex: 1, marginLeft: 12 }}>
                <Text style={{ fontSize: 14, fontWeight: "700", color: "#0f172a" }}>{item.name}</Text>
                <Text style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>ID: {item.studentId} · {item.time}</Text>
              </View>
