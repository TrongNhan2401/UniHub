...
export default function AdminWorkshopDetailPage() {
  ...
  const [workshop, setWorkshop] = useState({
    ...
    description:
      "Workshop này đi sâu vào tối ưu hóa hiệu suất, kiến trúc custom hooks và cách quản lý micro-frontends trong các ứng dụng quy mô lớn năm 2026.",
    ...
    aiSummary:
      "Workshop tập trung vào Module Federation và Atomic design. Người học sẽ nắm vững cách chia nhỏ hệ thống thành các Micro-frontends độc lập, tối ưu hóa Re-render thông qua các Custom Hooks nâng cao và quản lý State quy mô lớn.",
  });
  ...
  const [checkins] = useState([{ id: "SV001", name: "Nguyễn Văn A", checkinTime: "08:45 SA", method: "Mã QR" }]);
    ...
    <AdminShell activeTop="Quản lý Workshop">
      ...
      <div className="mb-8 flex gap-8 border-b border-slate-200">
        ...
        <TabButton
          ...
          label={`Đăng ký (${workshop.bookedSlots})`}
          icon={<Users size={18} />}
        />
        <TabButton
          active={activeTab === "checkins"}
          onClick={() => setActiveTab("checkins")}
          label={`Check-in (${workshop.checkedInCount})`}
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        ...
        <div className="min-h-[500px]">
          {activeTab === "overview" && (
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {/* AI Summary Card */}
              <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-blue-900 rounded-[2rem] p-8 text-white shadow-xl">
                <div className="relative z-10">
                  ...
                  <p className="text-blue-100 leading-relaxed font-medium">{workshop.aiSummary}</p>
                </div>
              ...
              <div className="bg-white p-8 rounded-[2rem] border border-slate-100 space-y-4">
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${workshop.status === "Published" ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-500"}`}
                  >
              ...
              <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] p-8 transition-all hover:border-blue-400 group text-center">
                <div
                  className={`mx-auto w-16 h-16 flex items-center justify-center rounded-full mb-4 transition-colors ${isUploading ? "bg-blue-100 animate-pulse" : "bg-white shadow-sm group-hover:bg-blue-50"}`}
                >
                  <Upload
                    className={isUploading ? "text-blue-600" : "text-slate-400 group-hover:text-blue-500"}
                    size={28}
                  />
                </div>
                <h3 className="font-bold text-slate-800">Cập nhật tài liệu AI (PDF)</h3>
                <p className="text-sm text-slate-500 mb-6">
                  Tải lên file PDF để AI cập nhật lại phần tóm tắt nội dung chuyên sâu.
                </p>
            ...
            <div className="space-y-4 animate-in slide-in-from-bottom-4">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-50 border-none text-sm font-medium"
              ...
              <AdminTable
                ...
                data={attendees.map((a) => [
                  <span className="font-bold text-slate-900" key={a.id}>
                    {a.id}
                  </span>,
                  a.name,
                  a.email,
                  <span
                    key={a.id}
                    className={`px-2 py-1 rounded-lg text-[10px] font-bold ${a.status === "Confirmed" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {attendeeStatusLabels[a.status]}
                  </span>,
                  a.time,
                ])}
              />
          ...
          {activeTab === "checkins" && (
            <div className="space-y-4 animate-in slide-in-from-bottom-4">
              <AdminTable
                ...
                data={checkins.map((c) => [
                  <span className="font-bold text-slate-900" key={c.id}>
                    {c.name}
                  </span>,
                  <span className="font-mono text-blue-600 font-bold" key={c.id}>
                    {c.checkinTime}
                  </span>,
                  c.method,
                  <button key={c.id} className="text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1 rounded-lg">
                    Hủy điểm danh
                  </button>,
                ])}
              />
        ...
        <aside className="space-y-6">
          <div className="sticky top-6 space-y-6">
            <div className="rounded-[2.5rem] bg-gradient-to-br from-slate-900 to-blue-900 p-8 text-white shadow-2xl overflow-hidden relative">
              <h3 className="text-lg font-bold mb-6 text-blue-400 flex items-center gap-2">
                <TrendingUp size={20} /> Hiệu suất
              </h3>
              <div className="space-y-6 relative z-10">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-500 uppercase mt-2 tracking-widest">
                    Đã đăng ký / {workshop.totalSlots} chỗ
                  </p>
                </div>
                <div className="pt-6 border-t border-slate-800 grid grid-cols-2 gap-4 text-center">
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase">Tham dự</p>
                    <p className="text-xl font-black text-emerald-400">{workshop.checkedInCount}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase">Tỷ lệ</p>
                    <p className="text-xl font-black text-white">{checkInRate.toFixed(0)}%</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-[2.5rem] shadow-2xl animate-in zoom-in-95">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                ...
                <FormGroup label="Tổng số chỗ">
                  <input
                    ...
                    onChange={(e) => setEditFormData({ ...editFormData, totalSlots: Number(e.target.value) })}
                  />
                </FormGroup>
              ...
              <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex items-center justify-between">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  {!editFormData.isFree && (
                    <input
                      type="number"
                      className="w-32 bg-white border border-slate-200 rounded-xl px-4 py-2 font-bold text-blue-600 outline-none"
            ...
            <div className="sticky bottom-0 bg-slate-50 px-8 py-6 flex justify-end gap-3 border-t border-slate-100">
              <button onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 font-bold text-slate-400">
                Hủy
              </button>
              <button
                onClick={handleSaveWorkshop}
                className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors"
              >
      ...
      <style jsx>{`
        .form-input {
          @apply w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:outline-none transition-all;
        }
      `}</style>
    </AdminShell>
  );
